...
# Import your hybrid architecture and config
from train.train_hybrid import HybridMultiScaleUNet, Config, prepare_datasets
...
cfg = Config()
cfg.FINAL_RESULTS_DIR = "final_results_hybrid_unet"
...
def plot_publication_psd(gt_list, pred_list, save_path):
    ...
    Shows frequency-domain fidelity of the Hybrid emulator.
    ...
    plt.semilogy(f_pred, p_pred, label='Hybrid U-Net Prediction', color='#ff7f0e', linestyle='--', linewidth=2)
...
def plot_inference_quad(X_sample, y_sample, pred_sample, sample_idx):
    ...
    # 4. Hybrid U-Net Prediction
    im3 = axes[3].imshow(pred_sample, cmap='RdBu_r', vmin=vmin, vmax=vmax)
    axes[3].set_title("(d) Hybrid U-Net Prediction", pad=10)
...
def main():
    print(f"\n{'='*60}")
    print("HYBRID MULTI-SCALE U-NET - PUBLICATION VISUALIZATION")
    print(f"{'='*60}\n")
    ...
    with open(summary_path, 'w') as f:
        f.write("="*60 + "\n")
        f.write("HYBRID MULTI-SCALE U-NET - PUBLICATION METRICS\n")
        f.write("="*60 + "\n\n")
        f.write(f"Model: Hybrid Multi-Scale U-Net with ASPP\n")
