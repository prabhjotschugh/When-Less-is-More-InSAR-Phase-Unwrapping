"""
COMBINED PUBLICATION VISUALIZATION — ALL 4 MODELS
IEEE GRSL Revision

Generates the combined PSD plot, combined CDF plot, 7-panel sample
visualizations, and efficiency/performance tables, sourced from the four
standardized-protocol training scripts (train_vanilla_unet.py,
train_enhanced_unet.py, train_attention_unet.py, train_hybrid.py).
...
OUTPUT_DIR = "GRSL_Final_visualizations"
os.makedirs(OUTPUT_DIR, exist_ok=True)
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")

MODEL_CONFIGS = {
    "Vanilla U-Net": {"config": ConfigVanilla(), "color": "#9467bd", "linestyle": ":", "linewidth": 2.0, "alpha": 0.9, "prepare_fn": prep_vanilla, "order": 1},
    "Attention U-Net": {"config": ConfigAttn(), "color": "#ff7f0e", "linestyle": "--", "linewidth": 2.2, "alpha": 0.9, "prepare_fn": prep_attn, "order": 2},
    "Enhanced U-Net": {"config": ConfigUNet(), "color": "#2ca02c", "linestyle": "-", "linewidth": 2.5, "alpha": 0.95, "prepare_fn": prep_unet, "order": 3},
    "Hybrid Multi-Scale": {"config": ConfigHybrid(), "color": "#d62728", "linestyle": "-.", "linewidth": 2.2, "alpha": 0.9, "prepare_fn": prep_hybrid, "order": 4},
}
...
def load_models():
    ...
    for name, config in MODEL_CONFIGS.items():
        ...
        if name == "Enhanced U-Net":
            model = EnhancedInSAR_UNet(cfg.IN_CHANNELS, cfg.OUT_CHANNELS, base_channels=cfg.BASE_CHANNELS, dropout=0.0)
        elif name == "Attention U-Net":
            model = AttentionInSAR_UNet(in_c=cfg.IN_CHANNELS, out_c=cfg.OUT_CHANNELS, base_channels=cfg.BASE_CHANNELS, dropout=0.0)
        elif name == "Hybrid Multi-Scale":
            model = HybridMultiScaleUNet(cfg.IN_CHANNELS, cfg.OUT_CHANNELS, base_channels=cfg.BASE_CHANNELS, dropout=0.0)
        else:
            model = VanillaInSAR_UNet(cfg.IN_CHANNELS, cfg.OUT_CHANNELS, base_channels=cfg.BASE_CHANNELS, dropout=0.0)
...
def compute_efficiency_metrics(models):
    ...
    for name, model in models.items():
        ...
        params_str = f"{param_count/1e6:.2f}M" if param_count >= 1e6 else f"{param_count/1e3:.2f}K"
        ...
        metrics[name] = {
            "parameters": param_count, "size_mb": param_size_mb, "flops": flops,
            "flops_str": flops_str, "params_str": params_str,
            "inference_time_ms": avg_time_ms, "inference_std_ms": std_time_ms,
        }
        print(f"  Parameters: {params_str} | Size: {param_size_mb:.2f} MB | FLOPs: {flops_str} "
              f"| Inference: {avg_time_ms:.2f} ± {std_time_ms:.2f} ms\n")

    with open(os.path.join(OUTPUT_DIR, "efficiency_metrics.txt"), "w") as f:
        f.write("="*80 + "\nMODEL EFFICIENCY COMPARISON — IEEE GRSL (Standardized Protocol)\n" + "="*80 + "\n\n")
        f.write("All 4 models trained under identical protocol: FP32, dropout=0.15,\n")
        f.write("weight_decay=1e-4, identical Huber+gradient loss, identical batch size/LR/epochs.\n")
        f.write("Only architecture (param count, layer types) differs between models.\n\n")
...
def plot_combined_psd(results):
    ...
    ax.semilogy(f_gt, p_gt, label="Ground Truth", color="#000000", linewidth=3.0, zorder=10)
    ...
    for name, config in sorted_models:
        ...
        ax.semilogy(f_pred, p_pred, label=name, color=config["color"], linestyle=config["linestyle"],
                    linewidth=config["linewidth"], alpha=config["alpha"], zorder=5)

    ax.set_xlabel("Spatial Frequency [cycles/pixel]", fontweight='bold')
    ax.set_ylabel("Power Spectral Density [$m^2$/freq]", fontweight='bold')
    ax.set_xlim(0, 0.5); ax.set_ylim(1e-7, 5e-2)
    ax.grid(True, which="both", alpha=0.3, linewidth=0.5)
    ax.grid(True, which="minor", alpha=0.15, linestyle=':', linewidth=0.3)
    ax.legend(loc="upper right", frameon=True, framealpha=0.95, edgecolor='black', fontsize=10)
    sns.despine()
    plt.tight_layout()
...
def plot_combined_cdf(results):
    ...
    for name, config in sorted_models:
        ...
        ax.plot(abs_errors, cdf, label=name, color=config["color"], linestyle=config["linestyle"],
                linewidth=config["linewidth"], alpha=config["alpha"])

    ax.set_xlabel("Absolute Error [cm]", fontweight='bold')
    ...
    ax.legend(loc="lower right", frameon=True, framealpha=0.95, edgecolor='black', fontsize=10)
    sns.despine()
    plt.tight_layout()
...
def plot_combined_visualization(vis_samples):
    ...
    for idx, sample in enumerate(vis_samples):
        ...
        axes[0].set_title("(a) Wrapped Phase", fontweight='bold')
        plt.colorbar(im0, ax=axes[0], fraction=0.046, pad=0.04).set_label('rad', fontsize=10)

        coh = X[2]
        im1 = axes[1].imshow(coh, cmap="viridis", vmin=0, vmax=1)
        axes[1].set_title("(b) Coherence", fontweight='bold')
        plt.colorbar(im1, ax=axes[1], fraction=0.046, pad=0.04).set_label('γ', fontsize=10)
        ...
        axes[2].set_title("(c) Ground Truth", fontweight='bold')
        plt.colorbar(im2, ax=axes[2], fraction=0.046, pad=0.04).set_label('m', fontsize=10)
        ...
        for i, (name, label) in enumerate(zip(model_order, labels)):
            ...
            axes[3 + i].set_title(label, fontweight='bold')
            plt.colorbar(im, ax=axes[3 + i], fraction=0.046, pad=0.04).set_label('m', fontsize=10)

        for ax in axes:
            ax.set_xticks([]); ax.set_yticks([]); ax.set_aspect('equal')

        plt.tight_layout()
        plt.savefig(os.path.join(OUTPUT_DIR, f"combined_sample_{idx}.pdf"), bbox_inches="tight", dpi=400)
        plt.close()
...
def main():
    print("\n" + "="*70 + "\nIEEE GRSL — COMBINED PUBLICATION RESULTS (Standardized Protocol)\n" + "="*70 + "\n")
    ...
    with open(os.path.join(OUTPUT_DIR, "performance_metrics.txt"), "w") as f:
        f.write("="*80 + "\nMODEL PERFORMANCE COMPARISON — IEEE GRSL (Standardized Protocol)\n" + "="*80 + "\n\n")
        f.write("Protocol: FP32 (no AMP), dropout=0.15 (all models), weight_decay=1e-4,\n")
        f.write("identical Huber+Sobel-gradient loss, identical LR/batch size/epochs/patience.\n")
        f.write("Only architecture differs.\n")
