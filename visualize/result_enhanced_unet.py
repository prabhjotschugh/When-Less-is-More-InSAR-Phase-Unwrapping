...
# Import your existing architecture and config
from train.train_enhanced_unet import EnhancedInSAR_UNet, Config, prepare_datasets

# ==========================================
# 1. STYLE & PUBLICATION CONFIGURATION
# ==========================================
# Setting professional font styles (Serif/Times)
plt.rcParams.update({
    "font.family": "serif",
...
cfg = Config()
cfg.FINAL_RESULTS_DIR = "final_results_unet"
...
def plot_publication_psd(gt_list, pred_list, save_path):
    ...
    Shows frequency-domain fidelity of the emulator.
    """
    ...
    plt.semilogy(f_pred, p_pred, label='U-Net Prediction', color='#ff7f0e', linestyle='--', linewidth=2)
...
def plot_inference_quad(X_sample, y_sample, pred_sample, sample_idx):
    ...
    axes[3].set_title("(d) U-Net Prediction", pad=10)
    ...
    plt.tight_layout()
    save_path = os.path.join(cfg.FINAL_RESULTS_DIR, f"inference_sample_{sample_idx}.pdf") # Save as PDF for vector quality
...
def main():
    ...
    plot_publication_psd(all_gt_m, all_pred_m, os.path.join(cfg.FINAL_RESULTS_DIR, "figure_psd_comparison.png"))
    
    errors_flat = np.concatenate(all_errors_cm)
    plot_error_cdf(errors_flat, os.path.join(cfg.FINAL_RESULTS_DIR, "figure_error_cdf.png"))

    print(f"\n{'='*50}")
    print(f"PUBLICATION RESULTS GENERATED SUCCESSFULLY")
    print(f"Location: {cfg.FINAL_RESULTS_DIR}")
    print(f"{'='*50}")
