...
class Config:
    ...
    MODEL_PATH = os.path.join(BASE_DIR, "insar_unet_best.pth")
    CHECKPOINT_PATH = os.path.join(BASE_DIR, "insar_checkpoint.pth")
    RESULTS_DIR = os.path.join(BASE_DIR, "results")
    VIZ_DIR = os.path.join(BASE_DIR, "results", "visualizations")
    TRAIN_VIZ_DIR = os.path.join(BASE_DIR, "results", "training_viz")
...
class SEBlock(nn.Module):
    ...
    def forward(self, x):  # Only takes x, not g
...
class EnhancedInSAR_UNet(nn.Module):
    """Enhanced U-Net with deeper encoder/decoder and better regularization"""
    ...
    def forward(self, x):
        ...
        d4 = self.up4(b)
        x4_att = self.att4(x4, d4)
        d4 = self.dec4(torch.cat([d4, x4_att], dim=1))
        
        d3 = self.up3(d4)
        x3_att = self.att3(x3, d3)
...
def calc_physics_loss(pred, target):
    ...
    # Penalize differences in spatial gradients (physics-aware)
...
def train_model(datasets, stats):
    print(f"\n{'='*60}\nSTEP 5: TRAINING WITH CHECKPOINT RESUMING\n{'='*60}")
    ...
    if cfg.RESUME_TRAINING and os.path.exists(cfg.CHECKPOINT_PATH):
        try:
            ...
            # Update stats if available in checkpoint
            if 'stats' in checkpoint:
            ...
            print(f"✓ Training history length: {len(history['train'])} epochs")
            print(f"{'='*60}\n")
        ...
        print("\nStarting fresh training (no checkpoint found or RESUME_TRAINING=False)...")
        model.apply(init_weights)
    
    # Create scheduler AFTER loading checkpoint
    # Calculate remaining steps for OneCycleLR
    ...
    scheduler = optim.lr_scheduler.OneCycleLR(
        ...
        pct_start=0.1,
        anneal_strategy='cos',
        last_epoch=total_steps - 1  # CRITICAL: Set last_epoch to resume correctly
    )
    ...
    if cfg.RESUME_TRAINING and os.path.exists(cfg.CHECKPOINT_PATH):
        try:
            ...
            print(f"  Scheduler will continue from step {total_steps}\n")
    ...
    print(f"  Checkpoint frequency: every {cfg.SAVE_CHECKPOINT_FREQ} epochs")
    print(f"  Visualizations every {cfg.VIZ_FREQUENCY} epochs\n")
    ...
    for epoch in range(start_epoch, cfg.EPOCHS):
        ...
        for batch_idx, (X, y) in enumerate(pbar):
            ...
            # Visualization on first batch of visualization epochs
        ...
        if (epoch + 1) % cfg.VIZ_FREQUENCY == 0:
            print(f"  Creating multi-sample visualization...")
        ...
        if early_stopping(avg_val, epoch):
            ...
            print(f"  Best validation loss: {early_stopping.best_score:.5f} at epoch {early_stopping.best_epoch+1}")
            break
    
    # Final checkpoint and curves
    save_checkpoint(epoch, model, optimizer, scheduler, early_stopping, history, best_loss, stats, total_steps)
