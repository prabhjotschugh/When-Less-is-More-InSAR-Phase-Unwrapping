...
#
# CHANGED FROM ORIGINAL: AMP/FP16 removed (now FP32 like all others),
# dropout unified to cfg.DROPOUT, weight decay unified, grad clip unified.
# ==========================================
...
class Config(BaseConfig):
    """Hybrid Multi-Scale U-Net config. ONLY paths/names differ from BaseConfig.
    NOTE: USE_AMP inherited as False, GRAD_CLIP inherited as 1.0 (was 0.5),
    WEIGHT_DECAY inherited as 1e-4 (was already 1e-4), DROPOUT inherited as 0.15 (was 0.20)."""
    MODEL_PATH = os.path.join(BaseConfig.BASE_DIR, "insar_hybrid_unet_best.pth")
    CHECKPOINT_PATH = os.path.join(BaseConfig.BASE_DIR, "insar_hybrid_unet_checkpoint.pth")
    RESULTS_DIR = os.path.join(BaseConfig.BASE_DIR, "results_hybrid_unet")
...
def save_training_visualization(X, y, pred, epoch, batch_idx, stats, save_dir):
    ...
    ax1.set_title('Input: Wrapped Phase', fontweight='bold'); ax1.axis('off')
    plt.colorbar(im1, ax=ax1, fraction=0.046, pad=0.04)

    ax2 = fig.add_subplot(gs[0, 1])
    im2 = ax2.imshow(coherence, cmap='gray', vmin=0, vmax=1)
    ax2.set_title('Input: Coherence', fontweight='bold'); ax2.axis('off')
    plt.colorbar(im2, ax=ax2, fraction=0.046, pad=0.04)

    ax3 = fig.add_subplot(gs[0, 2])
    im3 = ax3.imshow(u_comp, cmap='RdBu_r', vmin=-1, vmax=1)
    ax3.set_title('Input: LOS Up Component', fontweight='bold'); ax3.axis('off')
    plt.colorbar(im3, ax=ax3, fraction=0.046, pad=0.04)

    vmin_disp = min(y_np.min(), pred_np.min()); vmax_disp = max(y_np.max(), pred_np.max())

    ax4 = fig.add_subplot(gs[1, 0])
    im4 = ax4.imshow(y_np, cmap='RdBu_r', vmin=vmin_disp, vmax=vmax_disp)
    ax4.set_title('Ground Truth LOS Displacement', fontweight='bold'); ax4.axis('off')
    ...
    ax5.set_title('Hybrid U-Net Prediction', fontweight='bold'); ax5.axis('off')
    ...
    ax6.set_title('Error (GT - Pred)', fontweight='bold'); ax6.axis('off')
...
def create_multi_sample_visualization(model, dataloader, stats, epoch, save_dir, n_samples=5):
    ...
    for row, data in enumerate(all_data):
        ...
        if row == 0: ax5.set_title('Prediction', fontweight='bold')
        ...
        if row == 0: ax6.set_title('Error', fontweight='bold')
    ...
    fig.suptitle(f'Hybrid InSAR Emulator - Multi-Frame - Epoch {epoch+1}', fontweight='bold', y=0.995)
...
def prepare_datasets():
    """Hybrid is trained LAST in the recommended run order, so it expects the
    split file (created by Vanilla's first run) to already exist."""
    ...
    if not os.path.exists(cfg.SPLIT_FILE):
        raise FileNotFoundError(
            f"{cfg.SPLIT_FILE} not found. Run train_vanilla_unet.py first — it creates "
            f"the shared split file that all four models must use for a fair comparison."
        )
...
# ==========================================
# 5. HYBRID MULTI-SCALE MODEL ARCHITECTURE (the architectural variable:
#    SE blocks + ASPP multi-scale bottleneck + spatial attention gates)
# ==========================================
...
class ASPP(nn.Module):
    """Atrous Spatial Pyramid Pooling for multi-scale context (dilations 6/12/18 + global pool)."""
    def __init__(self, in_channels, out_channels, dropout=0.15):
        super().__init__()
        self.conv1 = nn.Sequential(nn.Conv2d(in_channels, out_channels, 1, bias=False),
                                    nn.BatchNorm2d(out_channels), nn.ReLU(inplace=True))
        self.conv2 = nn.Sequential(nn.Conv2d(in_channels, out_channels, 3, padding=6, dilation=6, bias=False),
                                    nn.BatchNorm2d(out_channels), nn.ReLU(inplace=True))
        self.conv3 = nn.Sequential(nn.Conv2d(in_channels, out_channels, 3, padding=12, dilation=12, bias=False),
                                    nn.BatchNorm2d(out_channels), nn.ReLU(inplace=True))
        self.conv4 = nn.Sequential(nn.Conv2d(in_channels, out_channels, 3, padding=18, dilation=18, bias=False),
                                    nn.BatchNorm2d(out_channels), nn.ReLU(inplace=True))
...
class HybridMultiScaleUNet(nn.Module):
    """Hybrid U-Net: SE blocks (encoder) + ASPP bottleneck (multi-scale dilated context)
    + spatial attention gates (decoder) + residual refinement head. 17.21M params at
    base_channels=32 — the most complex of the four. Dropout unified to cfg.DROPOUT."""
    def __init__(self, in_c=6, out_c=1, base_channels=32, dropout=0.15):
        ...
        self.bottleneck_conv = ResidualConvBlock(b*8, b*16, dropout=dropout)
        self.aspp = ASPP(b*16, b*16, dropout=dropout)
...
# ==========================================
# 7. TRAINING (FP32 — no AMP/GradScaler)
# ==========================================
...
def train_model(datasets, stats):
    print_protocol_banner("HYBRID MULTI-SCALE U-NET", cfg)

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"Device: {device} | AMP enabled: {cfg.USE_AMP}")
    ...
    if cfg.RESUME_TRAINING and os.path.exists(cfg.CHECKPOINT_PATH):
        try:
            ...
            print(f"✓ Resumed from epoch {start_epoch} | Best val: {best_loss:.5f}")
    ...
    for epoch in range(start_epoch, cfg.EPOCHS):
        ...
        for batch_idx, (X, y) in enumerate(pbar):
            ...
            # Plain FP32 — no autocast, no GradScaler (AMP removed for protocol fairness)
            pred = model(X)
            total_loss = calc_physics_loss(pred, y, cfg)
            total_loss.backward()
...
def evaluate(datasets, stats):
    print(f"\n{'='*60}\nSTEP 8: COMPREHENSIVE EVALUATION — HYBRID MULTI-SCALE U-NET\n{'='*60}")
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    print(f"Loaded model from epoch {checkpoint['epoch']+1} | Best val loss: {checkpoint['best_val_loss']:.5f}")
    ...
    print(f"\n{'='*60}\nHYBRID MULTI-SCALE U-NET - TEST METRICS\n{'='*60}")
    print(f"RMSE: {rmse:.3f} cm | MAE: {mae:.3f} cm | R²: {r2:.4f}")
    print(f"Std: {np.std(errors):.3f} cm | Median: {np.median(np.abs(errors)):.3f} cm")
    print(f"95th %: {np.percentile(np.abs(errors), 95):.3f} cm | 99th %: {np.percentile(np.abs(errors), 99):.3f} cm")
    print(f"{'='*60}\n")
...
if __name__ == "__main__":
    print("\n" + "="*60 + "\nHYBRID MULTI-SCALE InSAR U-NET (Standardized Protocol, FP32)\n" + "="*60)
