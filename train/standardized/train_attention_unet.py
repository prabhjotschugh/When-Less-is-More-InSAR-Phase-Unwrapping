...
# dropout unified to cfg.DROPOUT, weight decay unified, loss unified.
# ==========================================
...
class Config(BaseConfig):
    """Attention U-Net config. ONLY paths/names differ from BaseConfig.
    NOTE: USE_AMP is inherited as False — this model no longer trains in FP16.
    DROPOUT is inherited as 0.15 — no longer 0.20 as in the original script."""
    MODEL_PATH = os.path.join(BaseConfig.BASE_DIR, "insar_attn_unet_best.pth")
    CHECKPOINT_PATH = os.path.join(BaseConfig.BASE_DIR, "insar_attn_unet_checkpoint.pth")
    RESULTS_DIR = os.path.join(BaseConfig.BASE_DIR, "results_attn_unet")
...
def save_training_visualization(X, y, pred, epoch, batch_idx, stats, save_dir):
    ...
    ax5.set_title('Attention U-Net Prediction', fontweight='bold'); ax5.axis('off')
    ...
    fig.suptitle(f'Attention InSAR UNet - Epoch {epoch+1} | RMSE: {rmse*100:.2f} cm | MAE: {mae*100:.2f} cm',
...
def create_multi_sample_visualization(model, dataloader, stats, epoch, save_dir, n_samples=5):
    ...
    for row, data in enumerate(all_data):
        ...
        if row == 0: ax5.set_title('Attention U-Net Prediction', fontweight='bold')
    ...
    fig.suptitle(f'Attention InSAR UNet - Multi-Frame - Epoch {epoch+1}', fontweight='bold', y=0.995)
...
# ==========================================
# 5. ATTENTION MODEL ARCHITECTURE (the architectural variable: global self-attention
#    at bottleneck + spatial attention gates at skip connections)
# ==========================================
...
class ScaledDotProductAttention(nn.Module):
    """Spatial gated attention for U-Net decoder skip connections."""
    def __init__(self, F_g, F_l, F_int):
        super().__init__()
        self.W_g = nn.Sequential(nn.Conv2d(F_g, F_int, kernel_size=1, bias=True), nn.GroupNorm(8, F_int))
        self.W_x = nn.Sequential(nn.Conv2d(F_l, F_int, kernel_size=1, bias=True), nn.GroupNorm(8, F_int))
        self.psi = nn.Sequential(nn.Conv2d(F_int, 1, kernel_size=1, bias=True), nn.GroupNorm(1, 1), nn.Sigmoid())
        self.relu = nn.ReLU(inplace=True)
...
class AttentionInSAR_UNet(nn.Module):
    """Attention U-Net: SE blocks in encoder (matching baseline) + global self-attention
    at the bottleneck (4-head MHSA) + spatial gated attention at every skip connection.
    11.37M params at base_channels=32. Dropout unified to cfg.DROPOUT."""
    def __init__(self, in_c=6, out_c=1, base_channels=32, dropout=0.15):
        ...
        self.bottleneck = nn.Sequential(
            ...
            BottleneckAttention(b*16, num_heads=4, dropout=dropout)
        )
...
# ==========================================
# 6. PHYSICS LOSS (IDENTICAL formulation for ALL 4 models — Sobel-gradient version)
# ==========================================
...
# ==========================================
# 7. TRAINING WITH CHECKPOINT RESUMING (FP32 — no AMP/GradScaler)
# ==========================================
...
def train_model(datasets, stats):
    print_protocol_banner("ATTENTION U-NET", cfg)
    ...
    for epoch in range(start_epoch, cfg.EPOCHS):
        ...
        for batch_idx, (X, y) in enumerate(pbar):
            ...
            # Plain FP32 forward/backward — no autocast, no GradScaler (AMP removed for protocol fairness)
            pred = model(X)
...
def evaluate(datasets, stats):
    print(f"\n{'='*60}\nSTEP 8: COMPREHENSIVE EVALUATION — ATTENTION U-NET\n{'='*60}")
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    print(f"\n{'='*60}\nFINAL TEST METRICS — ATTENTION U-NET\n{'='*60}")
    ...
    with open(metrics_file, 'w') as f:
        f.write("="*60 + "\nAttention InSAR U-Net - Test Metrics\n" + "="*60 + "\n\n")
...
if __name__ == "__main__":
    print("\n" + "="*60 + "\nATTENTION InSAR U-Net (Standardized Protocol, FP32)\n" + "="*60)
